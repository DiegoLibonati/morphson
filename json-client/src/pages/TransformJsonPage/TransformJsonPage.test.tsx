...
describe("General Tests.", () => {
  mock.onGet("/json/inputs").reply(200, { data: MOCK_INPUT_JSONS });
  mock.onGet("/json/outputs").reply(200, { data: MOCK_OUTPUT_JSONS });
  ...
  beforeEach(() => {
    ...
    (useJSONContext as jest.Mock).mockReturnValue({
      ...
      outputJson: MOCK_OUTPUT_JSON_NULL_STATE,
      jsons: {
        inputJsons: MOCK_INPUT_JSONS,
        outputJsons: MOCK_OUTPUT_JSONS,
      },
...
describe("if calls are 200.", () => {
  const inputJsonSelected = MOCK_INPUT_JSONS[0];
  const outputJsonSelected = MOCK_OUTPUT_JSONS[0];
  ...
  test("It must choose an option from the json output select.", async () => {
    ...
    await user.selectOptions(selectOutputJsons, String(outputJsonSelected.id));
